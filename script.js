...
function buildAssetCategories() {
    ui.assetsSection.innerHTML = '';
    ui.assetsSection.appendChild(ui.fileInput); // Keep input
    ...
    ASSET_CATEGORIES.forEach(cat => {
        ...
        block.innerHTML = `
            ...
            <ul id="cat-list-${cat.id}" class="asset-list-ul"></ul>
        `;
...
function drawCanvas() {
    ...
    if (!AppState.loadedImage) { 
        ...
        ctx.font="20px Arial"; ctx.fillText("Velg tegning", w/2, h/2); 
        return; 
    }
...
async function handleFileUpload(event) {
    ...
    const ref = storage.ref().child(`users/${uid}/assets/${catId}/${Date.now()}_${file.name}`);
    ...
    try {
        ...
        await db.collection('users').doc(uid).collection('assets').add({
            originalName: file.name, url: url, type: file.type, category: catId,
...
function subscribeToAssets(uid) {
    ASSET_CATEGORIES.forEach(cat => { const list = document.getElementById(`cat-list-${cat.id}`); if(list) list.innerHTML = ''; });
...
function renderAssetItem(asset, id, catId) {
    ...
    if (AppState.selectedAsset && AppState.selectedAsset.id === id) li.classList.add('selected-item');
...
function animatePreview(timestamp) {
    ...
    if (frame && img) {
        const sx = frame.x; const sy = frame.y; const sw = frame.w; const sh = frame.h; 
        const padding = 0.9; const scaleX = (w * padding) / sw; const scaleY = (h * padding) / sh; const scale = Math.min(scaleX, scaleY); 
        const dw = sw * scale; const dh = sh * scale; const dx = (w - dw) / 2; const dy = (h - dh) / 2;
        ctx.imageSmoothingEnabled = false; ctx.drawImage(img, sx, sy, sw, sh, dx, dy, dw, dh);
...
async function createGif() {
    ...
    try {
        ...
        gif.render();
    } catch (error) { console.error(error); alert("GIF feil: " + error.message); ui.downloadGifBtn.innerText = originalText; ui.downloadGifBtn.disabled = false; }
}
